...
# Silence repetitive "use_label_encoder" deprecation messages
...
def load_data(
    data_path: str | pathlib.Path = THIS_DIR / "data.csv",
    labels_path: str | pathlib.Path = THIS_DIR / "labels.csv",
) -> Tuple[pd.DataFrame, pd.Series]:
    """Load matrices and return X, y after cleaning label typos/casing."""
    X = pd.read_csv(data_path, index_col=0)
    y = pd.read_csv(labels_path, index_col=0).squeeze("columns")
    if not isinstance(y, pd.Series):
        y = pd.Series(y)

    # --- Fix obvious typos / normalise text ---
    y = (
        y.astype(str)
        .str.strip()
        .str.lower()
        .replace({
            ...
            "colon cancer": "colon cancer",  # keep for consistency
            "breast cancer": "breast cancer",
            "lung cancer": "lung cancer",
        })
        .str.title()  # -> "Prostate Cancer", ...
    )
    return X, y
...
def preprocess_counts(X: pd.DataFrame) -> pd.DataFrame:
...
def binary_metrics(y_true: pd.Series | np.ndarray, y_pred: np.ndarray) -> Tuple[float, float]:
    """Return sensitivity (recall) and specificity for binary labels."""
...
# ---------------------------------------------------------------------------
# Evaluate a single one‑vs‑rest classification task
# ---------------------------------------------------------------------------
...
def evaluate_one_vs_rest(
    ...
    *,
    ...
    for train_idx, test_idx in skf.split(X, y_bin):
        ...
        sens, spec = binary_metrics(y_test, rf.predict(X_test))
        sens_rf.append(sens)
        spec_rf.append(spec)
        ...
        xgb = XGBClassifier(
            ...
            eval_metric="logloss",
            verbosity=0,            # no training spam
            n_jobs=-1,
            random_state=random_state,
        )
        xgb.fit(X_train, y_train)
        sens, spec = binary_metrics(y_test, xgb.predict(X_test))
...
def main() -> None:
    ...
    for cancer in y.unique():
        sens_rf, spec_rf, sens_xgb, spec_xgb = evaluate_one_vs_rest(
